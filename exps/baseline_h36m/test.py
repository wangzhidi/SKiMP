...
def regress_pred(model, pbar, num_samples, joint_used_xyz, m_p3d_h36):
    ...
    for (motion_input, motion_target, feature_target) in pbar:
        ...
        # # early_concatenate
        # motion_feats=motion_input.clone()
        # length = motion_feats.shape[-2]
        # # 66
        # feature_target = repeat(feature_target, 'B C -> B N C', N=length)
        # # 3300
        # # sketch_feature = sketch_feature.reshape(sketch_feature.shape[0],50,66)
        # motion_feats = torch.cat([motion_feats, feature_target], dim=-1)

        # modified
        # num_step = 1
        # if step == motion_target.shape[1]:
        #     num_step = 1
        # else:
        #     num_step = motion_target.shape[1] // step + 1
