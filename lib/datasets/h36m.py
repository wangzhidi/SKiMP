...
class H36MDataset(data.Dataset):
    ...
    def _get_h36m_files(self):
        ...
        for dataset in seq_names:
            # glob.glob() is used to find files, add a sort() after it for a fixed sequence sorted in a-z
    ...
    def __getitem__(self, index):
        ...
        if self.data_aug:
            if torch.rand(1)[0] > .5:
                idx_ = [i for i in range(motion.size(0)-1, -1, -1)]
                idx_ = torch.LongTensor(idx_)
                motion = motion[idx_]
        ...
        # GT
        # h36m_feature_target=h36m_motion_target[-1]
        # GT_residual
        # h36m_feature_target=h36m_motion_target[-1]-h36m_motion_input[-1]
